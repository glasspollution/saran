...
const Footer: React.FC<FooterProps> = ({
  ...
  return (
    <footer
      ref={footerRef}
      className="relative flex flex-col w-full h-full justify-between lg:h-screen select-none"
      style={{ backgroundColor: '#2A2822' }}
    >
              <div className="container mx-auto flex flex-col md:flex-row justify-between w-full gap-4 pb-24 pt-8 px-4 text-gray-100">
        <div className="space-y-2">
          <ul className="flex flex-wrap gap-4">
            {leftLinks.map((link, index) => (
              <li key={index}>
                <a href={link.href} className="text-base hover:text-orange-300 transition-colors duration-200">
                  {link.label}
                </a>
              </li>
            ))}
          </ul>
          <p className="text-base mt-4 flex items-center gap-x-1">
        ...
        <div className="space-y-4">
          ...
          <div className="text-right mt-4">
            <button className="text-base hover:underline hover:text-orange-300 transition-colors duration-200 inline-flex items-center">
              Back to top
            </button>
          </div>
        </div>
